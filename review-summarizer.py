...
def _analyze_numeric_ratings(review: Dict) -> Dict[str, str]:
...
def _analyze_salary_range(salary_range: str, currency: str) -> str:
...
def _analyze_text(text: str) -> Tuple[List[str], str]:
...
def analyze_review(review: Dict) -> Dict[str, List[str]]:
    ...
    rating_themes = _analyze_numeric_ratings(review)
    ...
    if 'compensation_range' in review and 'currency' in review:
        salary_sentiment = _analyze_salary_range(
    ...
    for field in text_fields:
        if field in review and review[field]:
            themes, sentiment = _analyze_text(review[field])
            for theme in themes:
                if field == 'pros':
...
def _generate_theme_text(theme: str, percentage: float) -> str:
...
def generate_summary(reviews: List[Dict]) -> Dict:
    ...
    for theme, (sentiment, percentage) in theme_sentiments.items():
        theme_text = _generate_theme_text(theme, percentage)
...
#     # Sample reviews (same as in the original example)
#     reviews = [
#         {
